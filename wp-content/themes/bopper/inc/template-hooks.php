 ...
 *
 */
function bopper_body_classes( $classes ) {
...
add_filter( 'body_class', 'bopper_body_classes' );

/**
 * Flush out the transients used in bopper_categorized_blog.
 *
 * @return bool Whether transients were deleted.
 */
function bopper_category_transient_flusher() {
...
add_action( 'delete_category', 'bopper_category_transient_flusher' );
add_action( 'save_post', 'bopper_category_transient_flusher' );
 ...
 */
function bopper_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', THEME_TEXT_DOMAIN ) . '...</a>';
}

add_filter( 'the_content_more_link', 'bopper_content_more_link' );
 ...
 * @return int
 */
function bopper_custom_excerpt_length( $length ) {
	return 20;
}

add_filter( 'excerpt_length', 'bopper_custom_excerpt_length', 999 );
 ...
 */
function bopper_get_the_content( $content ) {
	return ! empty( $content ) ? $content : false;
}

add_filter( 'the_content', 'bopper_get_the_content', 20 );
 ...
 * @author Bop Design
 */
function bopper_custom_mime_types( $mimes ) {
...
add_filter( 'upload_mimes', 'bopper_custom_mime_types' );
 ...
 * @author Bop Design
 */
function bopper_include_svg_icons() {
...
add_action( 'wp_footer', 'bopper_include_svg_icons', 9999 );
...
add_action( 'wp_head', 'bopper_add_og_tags' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function bopper_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}

add_action( 'wp_head', 'bopper_pingback_header' );
 ...
 */
function bopper_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'bopper_remove_archive_title_prefix' );

/**
 * Disables wpautop to remove empty p tags in rendered Gutenberg blocks.
 */
function bopper_disable_wpautop_for_gutenberg() {
...
add_filter( 'init', 'bopper_disable_wpautop_for_gutenberg', 9 );
