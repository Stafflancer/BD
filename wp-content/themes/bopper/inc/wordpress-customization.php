...
function bopper_unregister_tags() {
	unregister_taxonomy_for_object_type( 'post_tag', 'post' );
}

add_action( 'init', 'bopper_unregister_tags' );
 ...
 * @return void
 */
function bopper_filter_query( $query, $error = true ) {
	if ( is_search() ) {
...
//add_action( 'parse_query', 'bopper_filter_query' );
 ...
 * @return void
 */
function bopper_remove_search_widget() {
	unregister_widget( 'WP_Widget_Search' );
}

add_action( 'widgets_init', 'bopper_remove_search_widget' );
 ...
 * @return void
 */
function bopper_customize_register( $wp_customize ) {
	$wp_customize->remove_section( 'colors' );
	$wp_customize->remove_section( 'background_image' );
	$wp_customize->remove_section( 'header_image' );
}

add_action( 'customize_register', 'bopper_customize_register' );

function bopper_remove_unnecessary_wordpress_menus() {
	global $submenu;
...
add_action( 'admin_menu', 'bopper_remove_unnecessary_wordpress_menus', 999 );

/*** 7 - Show Featured Images in admin columns
 * @link https://www.isitwp.com/add-featured-thumbnail-to-admin-post-columns/
 * –––––––––––––––––––––––––––––––––––––––––––––––––– ***/
function posts_columns( $defaults ) {
	$defaults['bopper_post_thumbs'] = __( 'Featured Image' );

	return $defaults;
}

function posts_custom_columns( $column_name, $id ) {
	if ( 'bopper_post_thumbs' === $column_name ) {
 ...
 * @return mixed|void
 */
function bopper_get_site_icon_url( $url, $size, $blog_id ) {
...
add_filter( 'get_site_icon_url', 'bopper_get_site_icon_url', 10, 3 );
...
add_action( 'login_enqueue_scripts', 'bopper_login_logo' );

function bopper_login_logo_url() {
	return home_url();
}

add_filter( 'login_headerurl', 'bopper_login_logo_url' );