...
class GFP_Salesforce_OAuth2 {
	...
	private $addon_object;

	private $addon_settings_name;
	 ...
	 */
	public function __construct( $base_url = null, $client_key = null, $client_secret = null, $access_token = null, $access_token_expires = null, $callback = null, $scope = null, $refresh_token = null, $addon_object = null, $addon_settings_name = 'gravityformsaddon_gravityformssalesforce_settings' ) {
		...
		if  ( empty( $addon_object ) ) {

			global $gravityformssalesforce;

			$this->addon_object = $gravityformssalesforce->get_addon_object();

		}
		else {

			$this->addon_object = $addon_object;
		}

		$this->addon_settings_name = $addon_settings_name;
	}
	 ...
	 * @return string
	 */
	public function build_authorization_url( array $scope = array(), $scope_separator = ' ', $attach = null, $state_option_name = 'gravityformssalesforce_auth_state' ) {

		$auth_url = $this->_authorize_url;

		$auth_url .= '?client_id=' . $this->_client_id . '&redirect_uri=' . rawurlencode( $this->_callback );

		if ( ! empty( $scope ) ) {

			$auth_url .= '&scope=' . rawurlencode( implode( $scope_separator, $scope ) );
		...
		update_option( $state_option_name, $state );
			...
			if ( 401 == wp_remote_retrieve_response_code( $raw_response ) && 2 > $this->try_refreshing ) {
				...
				try {
					...
					if ( $this->request_access_token() && $this->access_token_updated() ) {
						...
						$current_settings = $this->addon_object->get_plugin_settings();

						$current_settings[ 'oauth_data' ] = $new_access_token_data;


						update_option( $this->addon_settings_name, $current_settings );
